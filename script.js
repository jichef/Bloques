/* ====== Parámetros ====== */
const GRID = 32;        // Tamaño de celda
const UNIT_SIZE = GRID; // ¡Ahora la ficha ocupa la celda exacta!
const COLORS = {
  unit: "blue",
  ten: "red",
  hundred: "green",
...
function drawGrid() {
  ...
  for (let x = 0; x <= w; x += GRID) {
    gridLayer.add(new Konva.Line({ points: [x,0,x,h], stroke: GRID_COLOR, strokeWidth: 1 }));
  }
  for (let y = 0; y <= h; y += GRID) {
    gridLayer.add(new Konva.Line({ points: [0,y,w,y], stroke: GRID_COLOR, strokeWidth: 1 }));
  }
  gridLayer.draw();
}
...
function centerPos(){ return { x: Math.floor(stage.width()/2), y: Math.floor(stage.height()/2) }; }
...
function createUnit(x,y){
  ...
  const rect=new Konva.Rect({ x:gx,y:gy,width:UNIT_SIZE,height:UNIT_SIZE,fill:COLORS.unit,draggable:true });
...
function createTen(x,y){
  const {x:gx,y:gy}=snapToGrid(x,y);
  const rect=new Konva.Rect({ x:gx,y:gy,width:10*GRID,height:GRID,fill:COLORS.ten,draggable:true });
...
function createHundred(x,y){
  const {x:gx,y:gy}=snapToGrid(x,y);
  const rect=new Konva.Rect({ x:gx,y:gy,width:10*GRID,height:10*GRID,fill:COLORS.hundred,draggable:true });
...
/* ====== Botones ====== */
function addUnit(){ const {x,y}=centerPos(); createUnit(x,y); }
function addTen(){ const {x,y}=centerPos(); createTen(x-5*GRID,y); }
function addHundred(){ const {x,y}=centerPos(); createHundred(x-5*GRID, y-5*GRID); }
